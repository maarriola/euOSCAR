...
constant DB_MAJOR = 2, DB_MINOR = 3   -- v2.3
...
--// Start Tone Skoda, 13. marec 2002.
global
--// End Tone Skoda
...
-- debug routine to detect corruption of free list
...
-- Get name of current selected database.
-- Returns "" if no database selected.
    integer index
...
-- close the current database
    ...
    index = find(current_db, db_file_nums)
	   db_names = db_names[1..index-1] & 
		      db_names[index+1..length(db_names)]
       db_file_nums = db_file_nums[1..index-1] &
		      db_file_nums[index+1..length(db_file_nums)]
    db_lock_methods = db_lock_methods[1..index-1] &
		      db_lock_methods[index+1..length(db_lock_methods)]
    current_db = -1 
end procedure
...
-- of the current database file
    ...
    key_pointers = key_pointers[1..key_location-1] & 
		   key_ptr &
		   key_pointers[key_location..length(key_pointers)]
	...
	-- avoid allocating extremely small blocks.

	new_size = 8 * floor(sqrt(total_recs+10))
...
-- delete record number rn in the current table
    ...
    key_pointers = key_pointers[1..key_location-1] & 
		   key_pointers[key_location+1..length(key_pointers)]
...
-- Thanks to Mike Nelson
    ...
    index = find(current_db, db_file_nums)
    new_path = db_names[index]
    db_close()
    
    fn = -1
    for i = 0 to 99 do
	-- try to find a temp name that isn't in use
	old_path = new_path[1..length(new_path)-3] & sprintf("t%d", i)
	fn = open(old_path, "r") 
	if fn = -1 then
	    exit
	else
    ...
    if fn != -1 then
	return DB_EXISTS_ALREADY -- better delete some temp files
    end if
...
--with 264474414 -- delete this statement if it causes an error
